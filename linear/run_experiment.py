...
from modules import LinearImputer
...
def main(dataset: str):
    ...
    def evaluate_loco(
        loader: DataLoader, model: LinearImputer, device: torch.device
    ):
    ...
    loco_df.to_csv(f"{DATASET_NAME}/linear_{DATASET_NAME}_loco.csv", index=False)
    ...
    def evaluate_mcar(
        ...
        for x, mask in tqdm(loader, desc="Evaluating MCAR", leave=False):
            ...
            batch_mcar_mask = batch_mcar_mask & mask.bool()
            batch_mcar_mask_np = batch_mcar_mask.cpu().numpy()
    ...
    mcar_df.to_csv(f"{DATASET_NAME}/linear_{DATASET_NAME}_mcar.csv", index=False)

    loco_pred_path = f"linear/{DATASET_NAME}/linear_{DATASET_NAME}_loco.csv"
    loco_out_dir = f"linear/{DATASET_NAME}/result_linear_{DATASET_NAME}_loco"
    run_external_evaluation(DATASET_NAME, loco_pred_path, loco_out_dir)

    mcar_pred_path = f"linear/{DATASET_NAME}/linear_{DATASET_NAME}_mcar.csv"
    mcar_out_dir = f"linear/{DATASET_NAME}/result_linear_{DATASET_NAME}_mcar"
