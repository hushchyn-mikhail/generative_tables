...
class TabularDataset(Dataset):
    def __init__(
        ...
        if self.is_valid:
            ...
            mask = ~pd.isna(self.table)
            random_mask = rng.random(mask.shape) > drop_rate
            mask_out = mask & random_mask
            dropped = mask & (~mask_out)
    ...
    def __getitem__(self, idx):
        ...
        if self.is_valid:
            row_dropped = self.dropped[idx : idx + 1, :].copy()
            row_dropped = row_dropped.astype(bool)
            row_mask = row_mask & (~row_dropped)

        row_table = torch.from_numpy(row_table[0])
        row_mask = torch.from_numpy(row_mask[0])

        if self.is_valid:
            row_dropped = torch.from_numpy(row_dropped[0])
            return row_table, row_mask, row_dropped
