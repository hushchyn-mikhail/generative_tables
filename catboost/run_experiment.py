...
def main(dataset: str):
    ...
    model: CatBoostImputer = joblib.load(
        f"{DATASET_NAME}/CatBoostImputer_{DATASET_NAME}.joblib"
    )
    ...
    def evaluate_loco(X: np.ndarray, model: CatBoostImputer):
        out = []
    ...
    loco_df.to_csv(f"{DATASET_NAME}/catboost_{DATASET_NAME}_loco.csv", index=False)
    ...
    mcar_df.to_csv(f"{DATASET_NAME}/catboost_{DATASET_NAME}_mcar.csv", index=False)

    loco_pred_path = f"catboost/{DATASET_NAME}/catboost_{DATASET_NAME}_loco.csv"
    loco_out_dir = f"catboost/{DATASET_NAME}/result_catboost_{DATASET_NAME}_loco"
    run_evaluation(DATASET_NAME, loco_pred_path, loco_out_dir)

    mcar_pred_path = f"catboost/{DATASET_NAME}/catboost_{DATASET_NAME}_mcar.csv"
    mcar_out_dir = f"catboost/{DATASET_NAME}/result_catboost_{DATASET_NAME}_mcar"
