...
if __name__ == '__main__':
    ...
    for trial in range(50):
        ...
        state_dict = torch.load(f'tabsyn/ckpt/{dataname}/model.pt', map_location=device)
        missing, unexpected = model.load_state_dict(state_dict, strict=False)
        if missing or unexpected:
            print(f'Checkpoint compatibility notice. Missing keys: {missing}; Unexpected keys: {unexpected}')
        ...
        mask = mask.to(torch.int).to(device)
        x_t = net.scale_noise(x_t.to(torch.float32), t_steps[0])
        ...
        with torch.no_grad():
            ...
            for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):
                ...
                if i < num_steps - 1:
                    for j in range(N):
                        n_curr = net.scale_noise(torch.randn_like(x).to(device), t_cur)
                        n_prev = net.scale_noise(torch.randn_like(x).to(device), t_next)
                        ...
                        n = net.scale_noise(torch.randn_like(x), (t_cur.pow(2) - t_next.pow(2)).sqrt())
