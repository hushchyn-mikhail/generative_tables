...
#     dev = next(pre_decoder.parameters()).device
#     syn_tensor = torch.tensor(syn_data, dtype=torch.float32, device=dev)
#     norm_input = pre_decoder(syn_tensor)
...
def split_num_cat_target(syn_data, info, num_inverse, cat_inverse, device):
    ...
    decoder_device = next(pre_decoder.parameters()).device
    syn_tensor = torch.as_tensor(syn_data, dtype=torch.float32, device=decoder_device)

    x_hat_num, x_hat_cat = pre_decoder(syn_tensor)
    ...
    syn_cat = torch.stack(syn_cat).t().detach().cpu().numpy()
