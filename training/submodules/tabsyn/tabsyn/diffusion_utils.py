...
def sample(net, num_samples, dim, num_steps = 50, device = 'cuda:0'):
    ...
    x_next = net.scale_noise(latents.to(torch.float32), t_steps[0])
...
def sample_step(net, num_steps, i, t_cur, t_next, x_next):
    ...
    gamma = min(S_churn / num_steps, np.sqrt(2) - 1) if S_min <= t_cur <= S_max else 0
    t_hat = net.round_sigma(t_cur + gamma * t_cur) 
    x_hat = x_cur + net.scale_noise(S_noise * randn_like(x_cur), (t_hat ** 2 - t_cur ** 2).sqrt())
    # Euler step.
...
class EDMLoss:
    ...
    def __call__(self, denoise_fn, data):
        ...
        sigma_eff = denoise_fn.get_effective_sigma(sigma)
