...
class CategoricalDecoderHead(nn.Module):
    """
    A stronger decoder head for categorical columns.

    The original TabSyn implementation used a single linear layer per categorical
    column. This head adds a small MLP with non-linearity and dropout, which
    helps turn the continuous latent token into sharper class logits.
    """

    def __init__(self, d_token: int, out_dim: int, hidden_dim: int, dropout: float = 0.0):
        super().__init__()
        self.norm = nn.LayerNorm(d_token)
        self.fc1 = nn.Linear(d_token, hidden_dim)
        self.fc2 = nn.Linear(hidden_dim, out_dim)
        self.dropout = nn.Dropout(dropout) if dropout > 0 else nn.Identity()
        self.activation = nn.ReLU()

        nn_init.xavier_uniform_(self.fc1.weight, gain=1 / math.sqrt(2))
        nn_init.zeros_(self.fc1.bias)
        nn_init.xavier_uniform_(self.fc2.weight, gain=1 / math.sqrt(2))
        nn_init.zeros_(self.fc2.bias)
...
class Reconstructor(nn.Module):
    def __init__(
        ...
        cat_decoder_hidden_factor: int = 4,
        cat_decoder_dropout: float = 0.0,
    ):
        ...
        self.cat_decoder_type = cat_decoder_type

        self.weight = nn.Parameter(Tensor(d_numerical, d_token))
        nn.init.xavier_uniform_(self.weight, gain=1 / math.sqrt(2))
        self.cat_recons = nn.ModuleList()

        hidden_dim = max(d_token * cat_decoder_hidden_factor, d_token)

        for d in categories:
            if cat_decoder_type == "linear":
                recon = nn.Linear(d_token, d)
                nn_init.xavier_uniform_(recon.weight, gain=1 / math.sqrt(2))
                nn_init.zeros_(recon.bias)
            elif cat_decoder_type == "mlp":
                recon = CategoricalDecoderHead(
                    d_token=d_token,
                    out_dim=d,
                    hidden_dim=hidden_dim,
                    dropout=cat_decoder_dropout,
                )
            else:
                raise ValueError(
                    f"Unknown cat_decoder_type={cat_decoder_type!r}. Expected 'linear' or 'mlp'."
                )
...
class Model_VAE(nn.Module):
    def __init__(
        ...
        self.Reconstructor = Reconstructor(
            ...
            categories,
            d_token,
            cat_decoder_type=cat_decoder_type,
            cat_decoder_hidden_factor=cat_decoder_hidden_factor,
            cat_decoder_dropout=cat_decoder_dropout,
        )
...
class Decoder_model(nn.Module):
    def __init__(
        ...
        self.Detokenizer = Reconstructor(
            d_numerical,
            categories,
            d_token,
            cat_decoder_type=cat_decoder_type,
