...
def make_dataset(
    ...
    # --- offline/compat fix: some info.json (from TabSyn datasets) don't have int_col_idx_wrt_num ---
    int_col_idx_wrt_num = info.get("int_col_idx_wrt_num", None)

    if int_col_idx_wrt_num is None:
        # try to infer from original integer column indices
        int_cols = list(info.get("int_col_idx", []))
        num_cols = list(info.get("num_col_idx", []))
        tgt_cols = list(info.get("target_col_idx", []))
        task = info.get("task_type", "")

        # positions among numerical feature columns (not original D)
        pos_in_num = [num_cols.index(c) for c in int_cols if c in num_cols]

        # TabDiff preprocess puts target(s) first for regression in X_num
        if task == "regression":
            int_col_idx_wrt_num = [len(tgt_cols) + p for p in pos_in_num]
        else:
            int_col_idx_wrt_num = pos_in_num
    # --- end fix ---
